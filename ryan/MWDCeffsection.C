...
//############################################################################  
   ...
   Int_t OpAngGate = 5; // 1= 20-30,
   Bool_t allentry  = 0;
   Int_t firstEntry = 3000000;
   Int_t nEntries=1000000;
   Double_t Qgate = 800;
   Bool_t histonoff = 0; // 0 = off  
   Bool_t ValidorTracked = 1; // 0 = Valid, 1 = tracked
   
//############################################################################
   Double_t AngSection[6] = {18.83, 28.42,38.18,48.16,58.36,68.78};
   Double_t xLSection[6]  = {-891.3, -629.8, -413.98,-222.47,-40.36, 144.07};
   Double_t xRSection[6]  = { 891.3,  629.8,  413.98, 222.47, 40.36,-144.07};
   
   Double_t xLGate[2] = {xLSection[OpAngGate -1],xLSection[OpAngGate]};
   Double_t xRGate[2] = {xRSection[OpAngGate -1],xRSection[OpAngGate]};
   ...
   if ( histonoff ){
      TCanvas * cMWDC = new TCanvas("cMWDC", "Charge of Tpla", 100, 50, 800, 400);
      cMWDC->Divide(2,1);
   }
...
//---------Get SMWDC image, should be one 1 instance
      ...
      if ( QTpla[0] > Qgate ){
         ...
         for( Int_t p = 0; p < nHit; p++){
            ...
            if ( track1->GetX() > xLGate[1] || track1->GetX() < xLGate[0] ) continue;
      ...
      if ( QTpla[1] > Qgate ){
         ...
         for( Int_t p = 0; p < nHit; p++){
            ...
            if ( track2->GetX() < xRGate[1] || track2->GetX() > xRGate[0] ) continue;
            Int_t NValidPlane = xy2 -> GetNPlaneValid();
            if ( NValidPlane >= 5 ) {
               mwdcBoth ++;
               //printf("%7.2f < %7.2f < %7.2f \n", xRGate[0],track2->GetX(),xRGate[1] );
            }
...
/*      clock.Stop("timer");
   ...
   printf("----------- %6.2f < Angle Section < %6.2f \n", AngSection[OpAngGate-1], AngSection[OpAngGate]);
