{
        ...
        //char * rootfile = "X_23F_ppcoin_r7_0619.root"; // all r
        //char * rootfile = "P_23F_r14_0824.root"; // all r //for asym
        //char * rootfile = "X_23F_r14_0824.root"; // all r    
        char * rootfile = "X2_23F_r14_0824.root"; // all rm with new dfMomt calculation 2016/04/06    
...
/**///======================================================== Browser or Canvas
        ...
        TCut cut23o = "TMath::Abs(pidAOQ*8-23)<0.4";
        TCut cut22o = "TMath::Abs(pidAOQ*8-22)<0.4";
        TCut cut21o = "TMath::Abs(pidAOQ*8-21)<0.4";
        TCut cut20o = "TMath::Abs(pidAOQ*8-20)<0.4";
        TCut cut19o = "TMath::Abs(pidAOQ*8-19)<0.4";
        TCut cut18o = "TMath::Abs(pidAOQ*8-18)<0.4";
...
/*++++++++++++++++++++++++++++++++++++++++++++++ E - theta */
/*
         tree->Draw("E2*0.8:theta2>>h1(50,10,80,50, 0, 400)", gate0 + gateZ + "-40<Ex && Ex<60", "colz" );
         h1->SetXTitle("theta2 [degree]");
         h1->SetYTitle("T2 [MeV]");
         h1->SetTitle("23F(p,2p)22O, -40<Ex<60 MeV");

/*++++++++++++++++++++++++++++++++++++++++++++++ theta1 = theta2 */

/*++++++++++++++++++++++++++++++++++++++++++++++ vertexZ */
/*
        tree->Draw("vertexZ>>h1(100,-100,300)", gateCry + gateTofS1 + gateNID + gateZ, "");
        
        h1->SetXTitle("vertex(Z) [mm]");
        h1->SetYTitle("count / 4 mm");
        

        TF1 * fit = new TF1("fit", "gaus(0) + gaus(3) + pol2(6)", -100, 300);
        fit->SetLineColor(1);
        fit->SetLineWidth(2);
        Double_t par[9]={250, 12, 15, 100,160, 12, 40, 0.022, -0.00123}; 
        fit->SetParameters(par);               
        h1->Fit("fit", "R");
        
        fit->GetParameters(par);
        
        Double_t rchisq=fit->GetChisquare()/fit->GetNDF();
        printf("reduced chi-squared = %f \n", rchisq);
        
        TF1 * g1 = new TF1("fit", "gaus(0)", -100, 400); g1->SetParameters(&par[0]); g1->SetLineColor(2); g1->Draw("same");
        TF1 * g2 = new TF1("fit", "gaus(0)", -100, 400); g2->SetParameters(&par[3]); g2->SetLineColor(3); g2->Draw("same");
        TF1 * g3 = new TF1("fit", "pol2(0)", -100, 400); g3->SetParameters(&par[6]); g3->SetLineColor(4); g3->Draw("same");
...
/*
        ...
        tree->Draw("theta2 >>h1(12,10,70)", gate  + "cut22o" + gateEx, "colz");
        tree->Draw("theta2>>h1c(12,10,70)", gatec + "cut22o" + gateEx, "colz");
        tree->Draw("theta2 >>h2(12,10,70)", gate  + "cut21o" + gateEx, "colz");
        tree->Draw("theta2>>h2c(12,10,70)", gatec + "cut21o" + gateEx, "colz");
        tree->Draw("theta2 >>h3(12,10,70)", gate  + "cut20o" + gateEx, "colz");
        tree->Draw("theta2>>h3c(12,10,70)", gatec + "cut20o" + gateEx, "colz");

        
        h1->SetTitle("23F(p,2p) | 22O"); h1->SetXTitle("theta1 [deg]"); h1->SetYTitle("count / 5 deg"); h1c->Scale(BGscale); 
        h2->SetTitle("23F(p,2p) | 21O"); h2->SetXTitle("theta1 [deg]"); h2->SetYTitle("count / 5 deg"); h2c->Scale(BGscale); 
        h3->SetTitle("23F(p,2p) | 20O"); h3->SetXTitle("theta1 [deg]"); h3->SetYTitle("count / 5 deg"); h3c->Scale(BGscale); 

        
        TH1F* m1 = new TH1F(*h1 - *h1c); m1->SetName("m1");m1->SetTitle("22O"); m1->SetLineColor(2);
        TH1F* m2 = new TH1F(*h2 - *h2c); m2->SetName("m2");m2->SetTitle("21O"); m2->SetLineColor(3); 
        TH1F* m3 = new TH1F(*h3 - *h3c); m3->SetName("m3");m3->SetTitle("20O"); m3->SetLineColor(4);
        ...
        m1->SetXTitle("theta1 [deg]");
        m1->SetYTitle("d.s.c [arb. unit]");
        
        m1->Draw("E");m2->Draw("E same");m3->Draw("E same");


/*++++++++++++++++++++++++++++++++++++++++++++++ angular correlation */
/*
        tree->Draw("theta2:theta1>>h1(55,15,70,55,15,70)" , gate , "colz");
        tree->Draw("theta2:theta1>>h1c(55,15,70,55,15,70)", gatec, "colz");
        
        tree->Draw("theta2:theta1>>h2(55,15,70,55,15,70)" , gate  + "cut22o", "colz");
        tree->Draw("theta2:theta1>>h2c(55,15,70,55,15,70)", gatec + "cut22o", "colz");
        tree->Draw("theta2:theta1>>h3(55,15,70,55,15,70)" , gate  + "cut21o", "colz");
        tree->Draw("theta2:theta1>>h3c(55,15,70,55,15,70)", gatec + "cut21o", "colz");
        
        h1->SetTitle("23F(p,2p) | target x Z");
        h1->SetXTitle("theta1 [deg]");
        h1->SetYTitle("theta2 [deg]");
        
        h1c->SetTitle("23F(p,2p) | carbon x Z");
        h1c->SetXTitle("theta1 [deg]");
        h1c->SetYTitle("theta2 [deg]");
        
        /*
        h1c->Scale(BGscale); 
        
        TH2F* m1 = new TH2F(*h1 - *h1c); m1->SetName("m1");m1->SetTitle("23F(p,2p)22O*");
        m1->SetXTitle("theta1 [deg]");
        m1->SetYTitle("theta2 [deg]");
...
/*
        ...
        //-------- count 
        Int_t sn22 = o22->Integral(13.,29.); Int_t n22 = o22c->Integral(13.,29.);
        Int_t sn21 = o21->Integral(12.,37.); Int_t n21 = o21c->Integral(12.,37.);
        Int_t sn20 = o20->Integral(12.,40.); Int_t n20 = o21c->Integral(12.,40.);
        
        printf(" 22O| SN:%4d, N:%4d, S:%4d(%3.0f) \n", sn22, n22, sn22-n22, sqrt(sn22+n22));
        printf(" 21O| SN:%4d, N:%4d, S:%4d(%3.0f) \n", sn21, n21, sn21-n21, sqrt(sn21+n21));
        printf(" 20O| SN:%4d, N:%4d, S:%4d(%3.0f) \n", sn20, n20, sn20-n20, sqrt(sn20+n20));
...
/*
        ...
        hAc->Scale(BGscale); TH1F* hAs = new TH1F(*hA - *hAc); hAs->SetTitle(""); hAs->Draw();
...
/*
        TF1* fit = new TF1("fit", "gaus(0)+gaus(3)+gaus(6)", 19.8, 24);
...
/*++++++++++++++++++++++++++++++++++++++++++++++ Momentum*/
        ...
        Ex00->Draw("k>>hk0c(40, 0, 400)", "xsec1p3", ""); hk0c->SetLineColor(4); 
        Ex00->Draw("k>>hk0d(40, 0, 400)", "xsec2s1", ""); hk0d->SetLineColor(6);
        Ex10->Draw("k>>hk1a(40, 0, 400)", "xsec1d5", ""); hk1a->SetLineColor(2); 
        Ex10->Draw("k>>hk1b(40, 0, 400)", "xsec1p1", ""); hk1b->SetLineColor(3); 
        Ex10->Draw("k>>hk1c(40, 0, 400)", "xsec1p3", ""); hk1c->SetLineColor(4); 
        Ex10->Draw("k>>hk1d(40, 0, 400)", "xsec2s1", ""); hk1d->SetLineColor(6); 
        Ex20->Draw("k>>hk2a(40, 0, 400)", "xsec1d5", ""); hk2a->SetLineColor(2); 
        Ex20->Draw("k>>hk2b(40, 0, 400)", "xsec1p1", ""); hk2b->SetLineColor(3); 
...
/*
        ...
        //TString plotTitle = "(23F,22O)"; 
        //TCut energy = "abs(Ex-0)<20";
        
        //TString plotTitle = "(23F,21O)"; 
        //TCut energy = "abs(Ex-10)<10";
